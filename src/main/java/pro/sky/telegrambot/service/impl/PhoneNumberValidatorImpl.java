...
public class PhoneNumberValidatorImpl {
    private static final String PHONE_PATTERN = "^(\\+7-9\\d{2}-\\d{3}-\\d{2}-\\d{2}|8-9\\d{2}-\\d{3}-\\d{2}-\\d{2}|89\\d{9}|\\+79\\d{9}) - (.+)$";
    ...
    public static String formatPhoneNumber(String phoneNumber) {
        ...
        if (phoneNumber.matches("^79\\d{9}$")) {
            return phoneNumber.replaceFirst("(\\+7)(\\d{3})(\\d{3})(\\d{2})(\\d{2})", "$1-$2-$3-$4-$5");
